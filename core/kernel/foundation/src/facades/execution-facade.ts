...
export class NoopExecutionFacade implements ExecutionFacade {
  execute(_plan: WorkflowPlan): Promise<ExecutionHandle> {
    return Promise.resolve({
      ...
      wait: () => Promise.resolve({ kind: 'ExecutionResult', schemaVersion: '1.0', executionId: '', executionRevision: 1, planId: '', metadata: { executionId: '', planId: '', startedAt: new Date().toISOString() }, termination: { reason: 'SUCCESS' }, stepRecords: [], journal: [], metrics: { totalDurationMs: 0, stepCount: 0, retryCount: 0, tokenCount: 0, providerLatencyMs: {}, startedAt: new Date().toISOString(), completedAt: new Date().toISOString() }, outputs: {}, producedAt: new Date().toISOString() } as import('@rohinik-org/compiler').ExecutionResult),
